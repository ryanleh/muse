...
fn main() {
    let weights_path = std::env::args()
        .nth(1)
        .expect("Pass weights path as first argument");
    let test_data = std::env::args()
        .nth(2)
        .expect("Pass test images as second argument");

    let mut rng = ChaChaRng::from_seed(RANDOMNESS);
    // Build network
    let mut network = construct_minionn(None, 1, &mut rng);
